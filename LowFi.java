...
public class LowFi extends Song {
	...
	private static String[] melodicInstruments = new String[] { "warm", "Helicopter", "Choir_Aahs", "Applause", "Rain",
			"Seashore", "Crystal", "Breath_Noise", "Goblins", };
	...
	public Pattern generateDrumline(int length) {
		...
		// Making a base and a metro layer based on the specified length ticks every
		// other
	...
	public String generateMelody(ArrayList<String> keys) {
		...
		case (0):
			...
			for (int count = 0; count < 4; count += 1) {
				theMelody += "V" + count + " I[" + melodicInstruments[(int) (Math.random() * melodicInstruments.length)]
						+ "] " + thePattern + "\n";
			}
			break;
		case (1):
	...
	public String generateBassLine(ArrayList<String> keys) {
		...
		theMelody += "V" + (int) (4) + " I[" + bassInstruments[(int) (Math.random() * bassInstruments.length)] + "] "
				+ thePattern + "\n";
